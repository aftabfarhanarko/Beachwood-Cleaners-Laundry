...
{/* SVG Icon Stickers matching reference screenshot */}
  ...
  return (
    <section className="py-10 sm:py-14 bg-[#F8FCFF]" id="services">
      <div className="max-container space-y-8">
        ...
        {/* 6 Cards Grid matching exact layout in screenshot */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {services.map((service) => {
            ...
            return (
              <div
                key={service.id}
                className="group relative rounded-3xl overflow-hidden shadow-xs hover:shadow-xl transition-all duration-300 bg-white border border-sky-100"
              >
                {/* Image Container */}
                ...
                {/* Floating Light Blue Card Tag Overlay matching exact reference design */}
                <div className="absolute bottom-3 left-3 right-3 sm:right-auto sm:max-w-[85%] bg-[#DDF2FF] border border-sky-200/60 rounded-2xl p-3 shadow-md flex items-center gap-3 backdrop-blur-xs group-hover:scale-102 transition-transform">
                  {/* White Circle Icon Badge */}
                  <div className="w-11 h-11 rounded-full bg-white flex items-center justify-center shadow-xs shrink-0 border border-sky-100">
                    <IconComponent />
                  </div>
