...
/*
  Animation strategy
  -------------------
  1) Static, non-changing sections (hero, filter/search controls) use
     whileInView with viewport:{ once: true }. They fire ONE time when
     first scrolled into view and never replay — scrolling up then back
     down does nothing, because there is no child-level whileInView to
     re-fire; only the single parent listens, and once:true removes it
     from observation after the first hit.

  2) The services grid is DATA-DRIVEN (filtered by category/search), so
     it can't rely purely on scroll — it also needs to animate when the
     filter changes even if the user never scrolls again. For that, each
     card uses initial/animate (not whileInView) inside AnimatePresence,
     keyed by service id, so cards animate in on first mount AND animate
     in/out cleanly whenever the filter results change. This never
     re-triggers from scrolling — only from actual content changes.
*/
  ...
  return (
    <div className="space-y-0 overflow-hidden bg-white">
      ...
      <motion.section
        ...
        className="py-12 sm:py-16 bg-gradient-to-r from-[#EBF7FD] via-[#FAFDFE] to-[#FFF4E8] text-center border-b border-sky-100"
      >
        <div className="max-container space-y-3">
          <motion.span
            variants={fadeUp}
            className="inline-block bg-[#EAF7FD] border border-sky-200/60 text-[#00A8E8] px-3.5 py-1 rounded-full text-xs font-extrabold uppercase tracking-wider"
          >
            Beachwood Fabric Care
          </motion.span>
          <motion.h1
            variants={fadeUp}
            className="text-2xl sm:text-2xl font-extrabold text-[#363C44] tracking-tight"
          >
            Our Professional <span className="text-[#F97316]">Services</span>
          </motion.h1>
          <motion.p
            variants={fadeUp}
            className="text-[#5B636B] text-xs sm:text-sm font-medium max-w-xl mx-auto leading-relaxed"
          >
      ...
      <section className="py-12 bg-[#FAFDFE] border-b border-sky-100" id="services">
        <div className="max-container space-y-8">
          ...
          >
            ...
            {/* Search Input */}
            <motion.div variants={fadeUp} className="relative w-full md:w-64 shrink-0">
              ...
              {filteredServices.map((service) => {
                ...
                return (
                  <motion.div
                    ...
                    className="group relative rounded-3xl overflow-hidden shadow-xs hover:shadow-2xl transition-shadow duration-500 bg-white border border-sky-100/90 flex flex-col justify-between"
                  >
                    <div>
          ...
          {filteredServices.length === 0 && (
            <motion.p
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-center text-sm font-semibold text-[#5B636B] py-10"
            >
